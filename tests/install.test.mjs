...
test('installs a pinned release and preserves an existing binary on rerun', async () => {
  ...
  try {
    const env = { ...process.env, HOME: home, PATH: `${fixture.binDir}:${process.env.PATH}`, SNIPSET_RELEASE_BASE_URL: 'https://fixture.invalid' };
    await run('bash', [script, '--version', '0.1.0', '--install-dir', installDir], { env });
    assert.equal(await readFile(join(installDir, 'snipset'), 'utf8'), '#!/bin/sh\nprintf "snipset 0.1.0\\n"\n');
    await run('bash', [script, '--version', '0.1.0', '--install-dir', installDir], { env });
  } finally {
...
test('rejects a checksum mismatch before replacing the installed binary', async () => {
  ...
  try {
    const env = { ...process.env, HOME: home, PATH: `${fixture.binDir}:${process.env.PATH}`, SNIPSET_RELEASE_BASE_URL: 'https://fixture.invalid' };
    await assert.rejects(run('bash', [script, '--version', '0.1.0', '--install-dir', installDir], { env }));
